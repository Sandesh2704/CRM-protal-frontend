...
export default function AddNewManager() {
    ...
    useEffect(() => {
      if (newManger.profileIMG) {
        const objectUrl = URL.createObjectURL(newManger.profileIMG);
        setPreviewSrc(objectUrl);
        return () => URL.revokeObjectURL(objectUrl); // Clean up the object URL
      }
    }, [newManger.profileIMG]);
    ...
    const inputHandler = (e) => {
     ...
     const handleSubmit = async (e) => {
      e.preventDefault();
      console.log('Submitting member data:', newManger);
      console.log('Team Leader ID:', founderId);
  
      const formData = new FormData();
  
      // Append user data to formData
      formData.append('founderId', founderId);
      for (const key in newManger) {
        formData.append(key, newManger[key]);
      }
  
      try {
          const response = await axios.post(`${process.env.REACT_APP_DOMAIN_URL}/founderManage/founder/add-manager`, formData, {
              headers: {
         ...
         setNewManger({
          ...
          const fetchResponse = await axios.get(`${process.env.REACT_APP_DOMAIN_URL}/founderManage/founder/${founderId}`);
 ...
 <div className='py-6 px-7'>
   <form onSubmit={handleSubmit}>
     <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3  lg:grid-rows-4 gap-x-7 gap-y-10'>
       ...
       {newManger.jobPosition === 'Employee' && (
         <>
           <TextInput
             type='text'
             label='Job Role*'
         ...
         <div className='flex gap-5 py-[16px] px-6 mt-4'>
           ...
           <div className='flex items-center gap-2'>
             <input
               ...
               name="gender"
               value="female"
               checked={newManger.gender === 'female'}
               onChange={inputHandler}
             />
